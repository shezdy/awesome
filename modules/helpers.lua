...
function helpers.contains(_table, _c)
    for _, c in ipairs(_table) do if _c == c then return true end end
    return false
end
...
function helpers.solid_rectangle_shadow(x_offset, y_offset)
    return function(cr, w, h)
        ...
        if y_offset < 0 then rect1, rect2 = rect2, rect1 end
...
helpers.rbar = function(width, height)
    return function(cr) gears.shape.rounded_bar(cr, width, height) end
end
...
function helpers.bold_text(txt) return "<b>" .. txt .. "</b>" end
