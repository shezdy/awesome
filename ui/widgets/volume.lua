...
function volume.update()
    util.async(volume.get_volume_cmd, function(stdout)
        ...
        if muted == "off" then
            volume.widget.markup = util.markup.color(
                theme.green,
                util.markup.font(theme.iconfont, "󰖁 ") .. ""
            )
        else
            if level == nil then
            ...
            elseif level < 10 then
                volume.widget.markup = util.markup.color(
                    theme.green,
                    util.markup.font(theme.iconfont, "󰕾 ") .. "0" .. level .. "%"
                )
            elseif level == 100 then
                volume.widget.markup = util.markup.color(
                    theme.green,
                    util.markup.font(theme.iconfont, "󰕾 ") .. "MAX"
                )
            else
                volume.widget.markup = util.markup.color(
                    theme.green,
                    util.markup.font(theme.iconfont, "󰕾 ") .. level .. "%"
                )
            end
        end
    end)
